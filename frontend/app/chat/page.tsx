...
import { LogOutIcon, BrainCircuit, CheckCircle, AlertCircle, Send, Mail, Settings, Info, Database, Trash2, RefreshCw, ExternalLink, ChevronDown, ChevronUp, Tag, Clock, Calendar, User, Paperclip, X, Search, BotMessageSquare } from "lucide-react";
...
const fetchEmailDetails = async (emailIds: string[], refreshToken: string): Promise<Record<string, EmailMetadata>> => {
  try {
    console.log(`Fetching details for ${emailIds.length} emails:`, emailIds);

    // Make API call to backend
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/emails/details`, {
      method: "POST",
      headers: {
...
const EmailCard = ({ email }: { email: EmailMetadata }) => {
  ...
  // Format date nicely
  const formattedDate = email.receivedTime ? getTimeDifference(email.receivedTime) : email.date;

  // Get sender name (without email address)
  const senderName = email.sender?.split("<")[0]?.trim() || email.sender || "Unknown";

  // Choose a color based on the first character of the subject or id
  const colorIndex = (email.subject?.charCodeAt(0) || email.id.charCodeAt(0)) % 5;
  const gradients = [
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-xl overflow-hidden border transition-all duration-300 ${hovered ? "shadow-lg border-gray-300 dark:border-gray-600 transform scale-[1.02]" : "shadow-md border-gray-200 dark:border-gray-700"}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      {/* Card Header */}
      <div className="p-4 pb-3">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3 w-full">
            <div className={`flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center`}>{email.hasAttachments ? <Paperclip className="h-5 w-5" /> : <Mail className="h-5 w-5" />}</div>
            <div className="flex-1 min-w-0">
              <h3 className="font-medium text-gray-900 dark:text-gray-100 truncate">{email.subject || "No Subject"}</h3>
          ...
          <button onClick={() => setExpanded(!expanded)} className={`ml-2 flex-shrink-0 rounded-full p-1.5 transition-colors ${expanded ? "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-100" : "text-gray-400 dark:text-gray-500 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800"}`} aria-label={expanded ? "Collapse email details" : "Expand email details"}>
            ...
            {email.labels && email.labels.length > 0 && (
              <div className="flex items-center space-x-1">
                ...
                <span className="truncate max-w-[100px]">
                  {email.labels[0]}
                  {email.labels.length > 1 ? ` +${email.labels.length - 1}` : ""}
                </span>
              </div>
            )}
          </div>
      ...
      {expanded && (
        <div className="px-4 pb-4">
          <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm text-gray-700 dark:text-gray-100 border border-gray-100 dark:border-gray-700">
            <p className="line-clamp-3">{email.snippet || "No preview available"}</p>
          </div>
          ...
          {email.labels && email.labels.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {email.labels.map((label, index) => (
                <span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100">
      ...
      <div className="relative h-10">
        <div className={`absolute inset-0 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-medium cursor-pointer hover:from-blue-600 hover:to-blue-700 transition-all flex items-center justify-center ${hovered ? "opacity-100" : "opacity-90"}`} onClick={() => openGmailWithEmail(email.threadId)}>
          <span>Open in Gmail</span>
          <ExternalLink size={14} className="ml-2" />

          {/* Hover Animation */}
          <div className={`absolute inset-0 bg-white opacity-0 ${hovered ? "animate-pulse-light" : ""}`}></div>
        </div>
      </div>
    </div>
  );
...
const EmailsContainer = ({ emailIds, visible, onClose, metadata = {}, isLoading = false }: { emailIds: string[]; visible: boolean; onClose: () => void; metadata?: Record<string, EmailMetadata>; isLoading?: boolean }) => {
  if (!visible || emailIds.length === 0) return null;

  // Create email metadata objects from IDs if not provided
  const emails: EmailMetadata[] = emailIds.map((id) => metadata[id] || createFallbackEmailMetadata(id));
    ...
    <div className="bg-gradient-to-br from-gray-50 to-blue-50/30 dark:from-gray-800 dark:to-blue-900/30 rounded-xl p-5 mt-4 border border-gray-200/70 dark:border-gray-700/70 shadow-sm animate-fadeIn">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h3 className="font-medium text-gray-900 dark:text-gray-100">Referenced Emails ({emails.length})</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400">These emails were used to generate the response</p>
          </div>
        </div>
        <button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-100 p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" aria-label="Close email references">
          <X size={16} />
        </button>
      </div>
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-10">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">Loading email details...</p>
          </div>
        </div>
      ) : (
...
export default function ChatBotPage() {
    ...
    const fetchEmailsData = async () => {
      if (emailIds && emailIds.length > 0) {
        ...
        try {
          ...
          console.log("Using refresh token:", refreshToken.substring(0, 5) + "...");

          // Fetch actual email details from Gmail
          const emailDetails = await fetchEmailDetails(emailIds, refreshToken);

          // Check if we got valid data
          const validEmails = Object.values(emailDetails).filter((email) => email.subject !== "Email Not Found");

          console.log(`Retrieved ${validEmails.length} valid emails out of ${emailIds.length} requested`);
  ...
  const fetchOnboardingProgress = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_RAG_URL}/progress?userId=${user?.id}`);
      const data = await response.json();
      ...
      if (!(data.phase === "waiting")) {
        ...
        if (count == 0) {
          ...
          let response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/onboardingRAG/optimisticRemove`, {
            method: "POST",
            credentials: "include",
  ...
  useEffect(() => {
    ...
    const shouldPoll = onboardingStatus.phase !== "complete" && onboardingStatus.phase !== "error" && onboardingStatus.phase !== "waiting" && user?.id;
    const isInProgress = onboardingStatus.total > 0 && onboardingStatus.total < 100;
  ...
  }, [onboardingStatus.phase, onboardingStatus.total, fetchOnboardingProgress, user?.id]);
  ...
  const toggleRag = async () => {
    ...
    try {
      ...
      const endpoint = newRagState ? "/api/onboardingRAG/enableRAG" : "/api/onboardingRAG/disableRAG";
  ...
  const getProgressStatusMessage = () => {
    switch (onboardingStatus.phase) {
      ...
      case "initializing":
        return isEnablingRag ? "Initializing RAG..." : "Preparing to disable RAG...";
      default:
        return "Processing...";
    }
  };
  ...
  let isProcessing = onboardingStatus.phase !== "complete" && onboardingStatus.phase !== "error" && onboardingStatus.phase !== "waiting";
  ...
  if (user?.provider === "microsoft") {
    return (
      <div className="fixed inset-0 bg-gray-900/95 backdrop-blur-md flex flex-col items-center justify-center z-50 text-white text-center px-6">
        ...
        <p className="text-lg text-gray-300 max-w-md mb-6">Outlook support for Recall is on its way. Hang tight — we're working on it!</p>
        <a href="/rules" className="bg-white text-black px-6 py-2 rounded-lg font-medium hover:bg-gray-200 transition-all duration-200">
          Back To Rules
        </a>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-b from-gray-50 to-gray-100">
        <div className="text-center p-8 max-w-md bg-white rounded-xl shadow-xl border border-gray-100">
          ...
          <Button className="bg-black hover:bg-gray-800 text-white px-6 py-2 rounded-lg transition-all duration-200 transform hover:scale-105" onClick={() => router.push("/")}>
            Return to Home
          </Button>
        </div>
      </div>
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center max-w-md w-full mx-4 transform animate-scaleIn">
            ...
            <h3 className="text-2xl font-semibold mt-8 mb-2">Loading workspace</h3>
            <p className="text-gray-500 text-center">Preparing your intelligent email assistant...</p>
          </div>
        </div>
      )}
        ...
        <div className="container mx-auto px-4 h-[calc(100vh-2rem)] py-4 max-w-7xl flex flex-col">
          ...
          <div className="space-y-4 mb-4">
            ...
            <div className="backdrop-blur-md rounded-2xl p-1">
              <div className="flex md:flex-row md:justify-between md:items-center gap-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 via-gray-700 to-gray-900 dark:from-white dark:via-gray-200 dark:to-white bg-clip-text text-transparent">Recall</h1>
            ...
            <div className="relative h-[90px]">
              ...
              <div className="absolute inset-0 bg-white/90 dark:bg-gray-800/90 rounded-xl shadow-sm border border-gray-300/80 dark:border-gray-700/80 overflow-hidden transition-all duration-300">
                <div className="flex items-center p-4">
                  <div className="flex-shrink-0 relative w-9 h-9">
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-br ${isProcessing ? "from-blue-600 to-blue-700" : onboardingStatus.phase === "complete" ? "from-green-600 to-green-700" : "from-gray-900 to-black dark:from-blue-600 dark:to-blue-700"} rounded-full p-2 transition-colors duration-500`}>{isProcessing ? <RefreshCw className="h-5 w-5 text-white animate-spin" /> : onboardingStatus.phase === "complete" ? <CheckCircle className="h-5 w-5 text-white" /> : <BrainCircuit className="h-5 w-5 text-white" />}</div>
                    {/* Circular progress indicator */}
                    {isProcessing && <div className="absolute -inset-1 rounded-full border-2 border-blue-400/30 border-t-blue-500"></div>}
                  </div>
                  ...
                  <div className="ml-3 flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="min-w-0 flex-1 pr-2">
                        ...
                        <h3 className="font-medium text-gray-900 dark:text-gray-100 truncate">Intelligent Email Search</h3>
                        <div className="flex items-center mt-0.5">
                          {isProcessing ? (
                            <p className="text-xs text-blue-600 dark:text-blue-400 flex items-center truncate">
                              <span className="animate-pulse mr-1.5 flex-shrink-0">•</span>
                              <span className="truncate">{getProgressStatusMessage()}</span>
                              <span className="ml-2 text-xs font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 px-2 py-0.5 rounded-full flex-shrink-0">{onboardingStatus.total}%</span>
                            </p>
                          ) : onboardingStatus.phase === "complete" ? (
                            <p className="text-xs text-green-600 dark:text-green-400 flex items-center truncate">
                              <span className="mr-1 flex-shrink-0">✓</span>
                              <span className="truncate">{getCompletionMessage()}</span>
                            </p>
                          ) : (
                            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{ragEnabled ? "Enabled" : "Disabled"}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-2 ml-2 flex-shrink-0 z-10">
                        ...
                        <Switch checked={ragEnabled} onClick={toggleRag} disabled={isProcessing} className={isProcessing ? "data-[state=checked]:bg-blue-500 data-[state=unchecked]:bg-blue-300" : onboardingStatus.phase === "complete" ? "data-[state=checked]:bg-green-500 data-[state=unchecked]:bg-green-300" : "data-[state=checked]:bg-black dark:bg-white data-[state=unchecked]:bg-gray-300"} />
                    ...
                    <div className="w-full h-1 bg-gray-100 dark:bg-gray-700/50 rounded-full mt-2 overflow-hidden">
                      {isProcessing && (
                        <div className="h-full bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-400 dark:to-blue-500 transition-all duration-700 ease-out rounded-full" style={{ width: `${onboardingStatus.total}%` }}>
              ...
              {isProcessing && <div className="absolute inset-0 bg-gradient-to-r from-blue-400/0 via-blue-400/30 to-blue-400/0 rounded-xl blur-sm animate-pulse-slow opacity-70 pointer-events-none"></div>}
              {onboardingStatus.phase === "complete" && <div className="absolute inset-0 bg-gradient-to-r from-green-400/0 via-green-400/30 to-green-400/0 rounded-xl blur-sm animate-pulse-slow opacity-70 pointer-events-none"></div>}
            </div>
          </div>
          ...
          <Card className="flex-1 flex flex-col w-full max-w-6xl mx-auto backdrop-blur-md bg-white/80 dark:bg-gray-900/80 shadow-xl border-gray-200/50 dark:border-gray-700/50 rounded-2xl overflow-hidden">
            ...
            <div className="relative flex-1 flex flex-col min-h-0">
              {isScrolled && <div className="absolute top-0 left-0 right-0 h-6"></div>}
              <CardContent
              ...
              >
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-2xl font-semibold mt-8 mb-3 text-gray-800 dark:text-gray-100">Welcome to Recall</h3>
                    <p className="max-w-md text-gray-500 dark:text-gray-400 mb-8">Ask me anything about your emails. I can help you find information, summarize content, and more.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                      {["Find emails from last week", "Summarize emails from John", "What's my next meeting?", "Show unread messages"].map((suggestion, i) => (
                        <button
                          key={i}
                    ...
                    {messages.map((m, index) => {
                      ...
                      return (
                        <div key={index} className="space-y-4 h-full">
                          <div className={`flex ${isUser ? "justify-end" : "justify-start"} ${isLast ? "animate-slideInFromBottom" : ""}`}>
                            {!isUser && (
                            ...
                            <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} max-w-[80%]`}>
                              <div className={`rounded-2xl px-5 py-3 shadow-md ${isUser ? "bg-black text-white dark:bg-white text-black dark:text-black rounded-tr-none" : "bg-white dark:bg-black text-black dark:text-white rounded-tl-none"}`}>
                                <div className="relative">
                              ...
                              <div className="flex items-center space-x-2 mt-1 px-2">
                                ...
                                {!isUser && emailIds && emailIds.length > 0 && index === messages.length - 1 && (
                                  <button onClick={() => setShowEmails(!showEmails)} className="ml-2 text-xs flex items-center space-x-1 text-blue-600 hover:text-blue-800 transition-colors">
                                    <Mail className="h-3 w-3" />
                                    <span>
                                      {showEmails ? "Hide" : "Show"} referenced emails ({emailIds.length})
                                    </span>
                                  </button>
                                )}
                              </div>
                            ...
                            {isUser && (
                              <div className="ml-3 flex-shrink-0">
                                <Avatar className="w-9 h-9 shadow-md transform hover:scale-105 transition-transform">
                                  <AvatarFallback className="bg-gradient-to-br from-gray-700 to-gray-900 text-white text-sm font-medium">{user?.email ? user.email.charAt(0).toUpperCase() : "U"}</AvatarFallback>
                                </Avatar>
                              </div>
                            )}
                          </div>
                          ...
                          {!isUser && index === messages.length - 1 && <EmailsContainer emailIds={currentEmailIds} visible={showEmails} onClose={() => setShowEmails(false)} metadata={emailMetadata} isLoading={isLoadingEmails} />}
                        </div>
                      );
                    })}
            ...
            <CardFooter className="border-t border-gray-200/50 dark:border-gray-700/50 p-4 bg-gradient-to-r from-gray-50/50 to-white/50 dark:from-gray-800/50 dark:to-gray-900/50">
              <form onSubmit={handleSubmit} className="flex w-full space-x-3">
                <div className="relative flex-grow">
                  <Input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your message..." className="w-full bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700 focus:border-black dark:focus:border-white focus:ring-black dark:focus:ring-white rounded-xl py-3 px-4 shadow-sm pr-12 text-gray-900 dark:text-gray-100" />
                  {/* <button
                  type="button"
                ...
                <Button type="submit" disabled={isTyping || !input.trim()} className={`bg-black dark:bg-white text-white dark:text-black rounded-xl px-6 transition-all duration-200 ${input.trim() ? "opacity-100 hover:shadow-lg" : "opacity-50 cursor-not-allowed"} transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:ring-offset-2 disabled:transform-none disabled:transition-none`}>
          ...
          <Dialog open={showPermissionDialog} onOpenChange={setShowPermissionDialog}>
            <DialogContent className="sm:max-w-md rounded-2xl p-0 overflow-hidden dark:bg-gray-900">
              <div className="bg-gradient-to-r from-gray-900 to-black dark:from-blue-600 dark:to-blue-700 text-white p-8">
                ...
                <DialogTitle className="text-2xl text-center font-semibold">Enable Email Intelligence</DialogTitle>
              </div>
              <div className="p-8 dark:bg-gray-900">
                <DialogDescription className="text-gray-600 dark:text-gray-300 mb-6 text-center">Enhance your email experience with AI-powered insights and intelligent responses.</DialogDescription>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 rounded-xl p-5 mb-8">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-200 leading-relaxed">RAG technology analyzes your emails securely to provide personalized, context-aware responses. Your data remains private and is never shared with third parties.</p>
                </div>
                ...
                <div className="space-y-5">
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-gray-100">Smart Responses</p>
                      <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Get AI-powered answers based on your email context</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-gray-100">Secure Processing</p>
                      <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">End-to-end encryption keeps your data safe</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 px-8 py-6 border-t border-gray-200 dark:border-gray-700">
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:justify-end">
                  <Button variant="outline" onClick={() => router.push("/rules")} className="w-full sm:w-auto border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl px-6 py-2.5">
                    Maybe Later
                  </Button>
                  <Button onClick={() => toggleRag()} className="w-full sm:w-auto bg-gradient-to-r from-gray-900 to-black dark:from-blue-600 dark:to-blue-700 hover:from-black hover:to-gray-900 dark:hover:from-blue-700 dark:hover:to-blue-800 text-white rounded-xl px-6 py-2.5 transform hover:scale-105 transition-all duration-200">
                    Enable Intelligence
                  </Button>
                </div>
              </div>
